...
func TestMultiArchBoot(t *testing.T) {
	...
	ctx := WithLogf(context.Background(), Logf(t.Logf))
	results, err := Run(ctx, iso, nil, timeout)
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
// don't drift away from convention: every EFIName ends with .EFI and
...
// TestRun_RejectsEmptyArches checks the explicit error path when the
// caller passes an empty slice (distinct from nil — nil substitutes
// DefaultArches and runs normally).
func TestRun_RejectsEmptyArches(t *testing.T) {
	_, err := Run(context.Background(), "/tmp/x.iso", []Arch{}, time.Second)
	if err == nil {
		t.Fatal("Run with empty arches returned nil error")
	}
	if !strings.Contains(err.Error(), "no arches") {
		t.Errorf("err=%q, want 'no arches'", err.Error())
	}
}
...
func TestRun_SkipsOnMissingISO(t *testing.T) {
	...
	results, err := Run(context.Background(), missing, nil, time.Second)
	if err != nil {
		t.Fatal(err)
	}
	for _, r := range results {
...
func TestRun_SkipsOnUnsetEnv(t *testing.T) {
	...
	results, err := Run(context.Background(), iso, nil, time.Second)
	if err != nil {
		t.Fatal(err)
	}
	for _, r := range results {
...
func TestRunOne_SkipsOnUnreadableFirmwareCode(t *testing.T) {
	...
	r := runOne(context.Background(), iso, time.Second, a, func(string, ...any) {})
	if r.Status != "skip" {
...
func TestRunOne_FailsWhenSerialNeverPrintsBanner(t *testing.T) {
	...
	t.Setenv("CLOUDBOOT_MB_FW_CODE", code)
	r := runOne(context.Background(), iso, 5*time.Second, a, func(string, ...any) {})
	if r.Status != "fail" {
...
func TestRunOne_ExpectFailPolarity(t *testing.T) {
	...
	xfailArch := Arch{
		Name: "test-xfail", EFIName: "BOOT.EFI", QEMUBin: "sh",
		QEMUArgsFor: func(string, string, string) []string {
			return []string{"-c", "echo nope; exit 0"}
		},
		CodeFWEnv: "CLOUDBOOT_MB_FW_CODE", WantSubstrs: []string{"DONE"},
		ExpectFail: true,
	}
	r := runOne(context.Background(), iso, 5*time.Second, xfailArch, func(string, ...any) {})
	if r.Status != "xfail" {
	...
	xpassArch := Arch{
		Name: "test-xpass", EFIName: "BOOT.EFI", QEMUBin: "sh",
		QEMUArgsFor: func(string, string, string) []string {
	...
	r2 := runOne(context.Background(), iso, 5*time.Second, xpassArch, func(string, ...any) {})
	if r2.Status != "xpass" {
...
// TestWithLogf_PassesLogf checks that the per-arch logger is invoked
// at least once when a Run reaches the QEMU-spawn stage. We verify
// using a fake-QEMU that exits cleanly.
func TestWithLogf_PassesLogf(t *testing.T) {
	dir := t.TempDir()
	iso := filepath.Join(dir, "fake.iso")
	if err := os.WriteFile(iso, []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	...
	a := Arch{
		Name: "test", EFIName: "BOOT.EFI", QEMUBin: "sh",
		QEMUArgsFor: func(iso, code, vars string) []string {
			return []string{"-c", "printf 'DONE\\n'; exit 0"}
		},
		CodeFWEnv: "CLOUDBOOT_MB_FW_CODE", WantSubstrs: []string{"DONE"},
	...
	var logCalls int
	ctx := WithLogf(context.Background(), Logf(func(string, ...any) { logCalls++ }))
	results, err := Run(ctx, iso, []Arch{a}, 5*time.Second)
	if err != nil {
		t.Fatal(err)
	}
	...
	if logCalls == 0 {
		t.Errorf("Logf was not invoked")
	}
}

// TestWithLogf_NilIsNoop ensures the helper survives a nil logger
// argument (the documented contract: return the original context).
func TestWithLogf_NilIsNoop(t *testing.T) {
	ctx := WithLogf(context.Background(), nil)
	// getLogf must still return a usable no-op function.
	getLogf(ctx)("ignored")
}

// TestRun_DefaultTimeoutIsApplied confirms the zero-timeout branch
// substitutes the package default. We exercise it by routing through
// the skip-on-missing-ISO path (which never spawns a process) so the
// test stays fast.
func TestRun_DefaultTimeoutIsApplied(t *testing.T) {
	dir := t.TempDir()
	missing := filepath.Join(dir, "no-such.iso")
	// Use a one-element arches slice so the loop fires exactly once.
	a := DefaultArches()[0]
	results, err := Run(context.Background(), missing, []Arch{a}, 0)
	if err != nil {
		t.Fatal(err)
	}
